...
const Cart = () => {
  ...
  return (
    <div className="py-8 px-4 md:px-8 lg:px-16 xl:px-32 mx-auto bg-[#F7FAFC] duration-200">
      ...
      {/* CART SECTION */}
      <div className="min-h-[50vh] flex flex-col lg:grid lg:grid-cols-4 gap-6">
        {/* CART ITEMS */}
        <div className="flex-1 lg:col-span-3">
          {cartItems.length === 0 ? (
            ...
            cartItems.map((item) => (
              <div
                key={item._id}
                className="flex flex-col sm:flex-row sm:items-center sm:justify-between border border-gray-300 p-4 rounded-lg mb-4 bg-white"
              >
        ...
        <div className="flex-1 space-y-6 lg:col-span-1">
          <div className="flex flex-col gap-2 p-4 rounded-lg h-fit border border-gray-300 bg-white">
            ...
            <div className="flex rounded-lg border border-gray-300 overflow-hidden">
              ...
              />
              <button className="p-2 border-l border-gray-300 text-blue-500 font-semibold cursor-pointer">
                Apply
              </button>
            </div>
          </div>
        ...
        <div className="mt-8 mb-8 bg-white p-6 rounded-xl border border-gray-300">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {savedItems.map((item) => (
              <div
                key={item._id}
                className="bg-white rounded-lg border border-gray-300 p-4 flex flex-col"
              >
                <img
                  src={item.image}
                  alt={item.name}
